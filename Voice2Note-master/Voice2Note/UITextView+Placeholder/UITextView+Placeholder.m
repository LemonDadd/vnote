...
-(void)k_setFrame:(CGRect)frame{
    ...
    [self placeholderTextView].frame = (CGRect){self.contentInset.left,0,self.bounds.size};
}
...
-(void)addplaceholderTextView{
    UITextView *textView = [[UITextView alloc]initWithFrame:(CGRect){self.contentInset.left,0,self.frame.size.width,self.frame.size.width}];
