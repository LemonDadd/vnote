...
- (void)createUI {
    ...
    [_rightBtn handleControlEvent:UIControlEventTouchUpInside withBlock:^(UIButton *sender) {
        if (weakSelf.password.length == 0) {
            ...
            if (weakSelf.password.length == 0) {
                [weakSelf.rightBtn setTitle:@"取消" forState:UIControlStateNormal];
            }
        }
    ...
    _titleLabel.textColor = [UIColor systemColor];
    ...
    [_board setDidInputNumber:^(NSString *number){
        if (weakSelf.password.length < 6) {
            ...
            [weakSelf.rightBtn setTitle:@"删除" forState:UIControlStateNormal];
            [weakSelf.collectionView reloadData];
            if (weakSelf.password.length == 6) {
...
- (void)dismissComplete:(void(^)())callBack {
    if ([_titleString isEqualToString:@"请输入密码"] && !_password.length) {
        [CustomView alertMessage:_titleString view:self.view];
        return;
    }
    WEAK_SELF
