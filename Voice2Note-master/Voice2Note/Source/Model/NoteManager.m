...
{
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask, YES);
    NSString *uniquePath=[[paths objectAtIndex:0] stringByAppendingPathComponent:note.noteID];
    [[NSFileManager defaultManager] removeItemAtPath:uniquePath error:nil];
}

- (void)deleteAllNote {
    for (VNNote *note in [self readAllNotes]) {
...
- (VNNote *)todayNote
{
  ...
  for (VNNote *note in notes) {
    if ([NSDate isSameDay:[NSDate jk_dateWithString:note.createdDate format:@"yy-MM-dd HH:mm:ss"] andDate:[NSDate date]]) {
      return note;
    }
  }
  return nil;
